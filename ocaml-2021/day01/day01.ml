...
let part_1 = function
  | first :: measurements ->
      let (n, _) = List.fold measurements
      ~init: (0, first)
      ~f: (fun (n, last) current -> 
...
let part_2 measurements =
  windows 3 measurements
  |> List.(map ~f: (fold ~init: 0 ~f: (+)))
  |> part_1

let () =
  let lines =
    (List.map @@ In_channel.read_lines input_path)
    ~f: int_of_string
  in
